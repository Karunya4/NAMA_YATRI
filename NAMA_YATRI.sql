...
select loc_from, trip_count
from (
    select loc_from, count(distinct tripid) as trip_count,
           rank() over (order by count(distinct tripid) desc) as rnk
    from trips
    group by loc_from
) b
